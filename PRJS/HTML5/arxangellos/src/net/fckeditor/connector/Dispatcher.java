...
public class Dispatcher {
	...
	GetResponse doGet(final HttpServletRequest request) {
		...
		if (!Command.isValidForGet(context.getCommandStr()))
			...
			if ((command.equals(Command.GET_FOLDERS) || command.equals(Command.GET_FOLDERS_AND_FILES))
				...
				try {
					if (command.equals(Command.CREATE_FOLDER)) {
					...
					else if (command.equals(Command.DELETE_FILE )  ){
						...
						String deleteFileNameStr = request.getParameter("DeleteFileName");
						
						
						
						if ( deleteFileNameStr!= null && deleteFileNameStr.length() > 2 ){
							
							
							logger.info("TRY to deleteFileNameStr = "+ deleteFileNameStr);
		...
		String  deleteFile = 
		Constants.FCK_EDITOR_IMG_HEAD_PATH +"/userfiles/"+  userObj.getUname() + "/image/"+ deleteFileNameStr ;
						
						
						logger.info ("DELETE IT HERE  this file:   = "+ deleteFile  );
							
		DeleteFile.delete(deleteFile);
						
		getResponse = new GetResponse(   
					GetResponse.EN_OK, 
				"USER = "+ userObj.getUname() + "  deleteFile = "+ deleteFile );
